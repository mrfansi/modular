...
class MakeModule extends Command
{
	...
	public function handle()
	{
		...
		$this->module_namespace = config('platform.modules_namespace', 'Modules');
		$this->composer_namespace = config('platform.modules_vendor') ?? Str::kebab($this->module_namespace);
	...
	protected function writeStubs()
	{
		...
		$tests_base = config('platform.tests_base', 'Tests\TestCase');
	...
	protected function updateCoreComposerConfig()
	{
		...
		$module_config = [
			'type' => 'path',
			'url' => str_replace('\\', '/', config('platform.modules_directory', 'platform')).'/*',
			'options' => [
				'symlink' => true,
			],
		];
	...
	{
		if (is_array($custom_stubs = config('platform.stubs'))) {
			return $custom_stubs;
		}
